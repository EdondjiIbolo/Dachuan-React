...
import { useCompletedQuotes } from "../../../Hooks/useCompletedQuotes";
import { Loading } from "../../Loading";
export function QuoteComplete() {
  const { user } = useUser();
  const { email } = user;
  const { quotes, loading } = useCompletedQuotes({ email });
        ...
        <main className="w-full h-full pt-5">
          <section className="bg-white flex flex-col gap-4  rounded shadow-sm p-4">
            <header className="flex justify-between items-center">
              <h2 className="font-semibold text-xl py-2">Completed Quotes</h2>
